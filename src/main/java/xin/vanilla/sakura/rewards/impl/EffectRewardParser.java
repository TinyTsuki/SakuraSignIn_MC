...
public class EffectRewardParser implements RewardParser<MobEffectInstance> {
    ...
    public @NonNull MobEffectInstance deserialize(JsonObject json) {
        ...
        try {
            ...
            Optional<Holder<MobEffect>> effect = ForgeRegistries.MOB_EFFECTS.getHolder(ResourceLocation.parse(effectId));
            if (effect.isEmpty()) {
    ...
    public static String getId(MobEffectInstance instance) {
        return getId(instance.getEffect().value()) + " " + instance.getDuration() + " " + instance.getAmplifier();
    }
    ...
    public static MobEffect getEffect(String id) {
        ...
        return ForgeRegistries.MOB_EFFECTS.getValue(ResourceLocation.parse(resourceId));
    }
