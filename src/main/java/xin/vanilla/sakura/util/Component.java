...
public class Component implements Cloneable, Serializable {
    ...
    public Style getStyle() {
        ...
        if (!isColorEmpty()) {
            TextFormatting code = TextFormatting.getByCode(StringUtils.argbToMinecraftColor(getColor()).replace("§", "").toCharArray()[0]);
            if (code != null) {
    ...
    public static Component deserialize(JsonObject jsonObject) {
        ...
        if (jsonObject.has("hoverEvent.action") && jsonObject.has("hoverEvent.value")) {
            result.setHoverEvent(new HoverEvent(HoverEvent.Action.valueOf(jsonObject.get("hoverEvent.action").getAsString()), ITextComponent.Serializer.fromJson(jsonObject.get("hoverEvent.value"))));
        }
    ...
    public static JsonObject serialize(Component reward) {
        ...
        if (reward.getHoverEvent() != null) {
            result.addProperty("hoverEvent.action", reward.getHoverEvent().getAction().getName());
            result.add("hoverEvent.value", ITextComponent.Serializer.toJsonTree(reward.getHoverEvent().getValue()));
        }
