...
public class OperationButton {
    ...
    public void render(double mouseX, double mouseY, boolean renderPopup, int keyCode, int modifiers) {
        if (customRenderFunction != null) {
            ...
            if (bgColor != 0) {
                AbstractGuiUtils.fill((int) (baseX + coordinate.getX() * scale), (int) (baseY + coordinate.getY() * scale), (int) (coordinate.getWidth() * scale), (int) (coordinate.getHeight() * scale), bgColor);
            }
            // 绘制纹理
            if (this.isHovered() && this.getTremblingAmplitude() > 0) {
                AbstractGuiUtils.renderTremblingTexture(this.texture, textureCoordinate, coordinate, this.baseX, this.baseY, this.scale, true, this.getTremblingAmplitude());
            } else {
                AbstractGuiUtils.renderRotatedTexture(this.texture, textureCoordinate, coordinate, this.baseX, this.baseY, this.scale, this.rotatedAngle, this.flipHorizontal, this.flipVertical);
            }
            // 绘制前景颜色
            int fgColor = this.getForegroundColor();
            if (fgColor != 0) {
                AbstractGuiUtils.fill((int) (baseX + coordinate.getX() * scale), (int) (baseY + coordinate.getY() * scale), (int) (coordinate.getWidth() * scale), (int) (coordinate.getHeight() * scale), fgColor);
            }
        }
        if (renderPopup) {
