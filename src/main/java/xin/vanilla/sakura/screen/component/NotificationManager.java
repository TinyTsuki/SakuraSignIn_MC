...
public class NotificationManager {
    ...
    public static class Notification {
        ...
        @OnlyIn(Dist.CLIENT)
        private void render(GuiGraphics graphics, double[] preY, double[] preHeight) {
            // 计算动画和时间状态
            ...
            // 计算目标坐标
            double targetX;
            double targetY = switch (position) {
                case TOP_LEFT -> {
                    targetX = this.getMargin();
                    yield this.getMargin() + preY[0] + preHeight[0];
                }
                case TOP_CENTER -> {
                ...
                case BOTTOM_LEFT -> {
                    targetX = this.getMargin();
                    yield (preY[0] == 0 ? screenHeight : preY[0]) - this.getMargin() - height;
                }
                case BOTTOM_CENTER -> {
                    targetX = (screenWidth - width) / 2;
                    yield (preY[0] == 0 ? screenHeight : preY[0]) - this.getMargin() - height;
                ...
                case CENTER -> {
                    targetX = (screenWidth - width) / 2;
                    yield (screenHeight - height) / 2;
                }
                default -> {
                    targetX = this.getMargin();
                    yield this.getMargin();
                }
            };
            ...
            AbstractGuiUtils.setDepth(graphics, AbstractGuiUtils.EDepth.POPUP_TIPS);
            // 在计算完的坐标位置绘制消息框背景
            AbstractGuiUtils.fill(graphics, (int) x, (int) y, (int) width, (int) height, this.getBgColor(), this.getRadius());
            // 绘制消息框描边
            AbstractGuiUtils.fillOutLine(graphics, (int) x, (int) y, (int) width, (int) height, this.getBorderSize(), this.getBorderColor(), this.getRadius());
            // 绘制消息文字
            AbstractGuiUtils.drawLimitedText(text.setGraphics(graphics), x + this.getPadding(), y + this.getPadding(), 0, 0, AbstractGuiUtils.EllipsisPosition.MIDDLE);
    ...
    public void render(GuiGraphics graphics) {
        if (graphics == null) graphics = new GuiGraphics(Minecraft.getInstance(), new RenderBuffers().bufferSource());
        for (Map.Entry<EPosition, List<Notification>> entry : notifications.entrySet()) {
            ...
            for (int i = 0; i < entry.getValue().size(); i++) {
                ...
                notification.render(graphics, y, h);
            }
        }
        // 移除已经执行完成的通知
