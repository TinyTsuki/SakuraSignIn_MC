...
public class NotificationManager {
    ...
    public static class Notification {
        ...
        @OnlyIn(Dist.CLIENT)
        private void render(MatrixStack matrixStack, double[] preY, double[] preHeight) {
            // 计算动画和时间状态
            ...
            if (this.getLastIndex() > 0 && this.getIndex() == 0 && this.getLastY() != y) {
                ...
                switch (this.getPosition()) {
                    ...
                    case TOP_RIGHT:
                        this.setIndex(1);
                        y = Math.max(y, this.lastY - v);
                        break;
                    case BOTTOM_LEFT:
                    case BOTTOM_CENTER:
                    case BOTTOM_RIGHT:
                        this.setIndex(1);
                        y = Math.min(y, this.lastY + v);
                        break;
                }
            }
            ...
            // 在计算完的坐标位置绘制消息框背景
            AbstractGuiUtils.fill(matrixStack, (int) x, (int) y, (int) width, (int) height, this.getBgColor(), this.getRadius());
            // 绘制消息框描边
            AbstractGuiUtils.fillOutLine(matrixStack, (int) x, (int) y, (int) width, (int) height, this.getBorderSize(), this.getBorderColor(), this.getRadius());
            // 绘制消息文字
            AbstractGuiUtils.drawLimitedText(text.setMatrixStack(matrixStack), x + this.getPadding(), y + this.getPadding(), 0, 0, AbstractGuiUtils.EllipsisPosition.MIDDLE);
    ...
    public void render(MatrixStack matrixStack) {
        if (matrixStack == null) matrixStack = new MatrixStack();
        for (Map.Entry<EPosition, List<Notification>> entry : notifications.entrySet()) {
            ...
            for (int i = 0; i < entry.getValue().size(); i++) {
                ...
                notification.render(matrixStack, y, h);
            }
        }
        // 移除已经执行完成的通知
