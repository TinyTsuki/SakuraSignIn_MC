...
public class NotificationManager {
    ...
    public static class Notification {
        ...
        @OnlyIn(Dist.CLIENT)
        private void render(PoseStack poseStack, double[] preY, double[] preHeight) {
            // 计算动画和时间状态
            ...
            // 在计算完的坐标位置绘制消息框背景
            AbstractGuiUtils.fill(poseStack, (int) x, (int) y, (int) width, (int) height, this.getBgColor(), this.getRadius());
            // 绘制消息框描边
            AbstractGuiUtils.fillOutLine(poseStack, (int) x, (int) y, (int) width, (int) height, this.getBorderSize(), this.getBorderColor(), this.getRadius());
            // 绘制消息文字
            AbstractGuiUtils.drawLimitedText(text.setPoseStack(poseStack), x + this.getPadding(), y + this.getPadding(), 0, 0, AbstractGuiUtils.EllipsisPosition.MIDDLE);
    ...
    public void render(PoseStack poseStack) {
        ...
        for (Map.Entry<EPosition, List<Notification>> entry : notifications.entrySet()) {
            ...
            for (int i = 0; i < entry.getValue().size(); i++) {
                ...
                notification.render(poseStack, y, h);
            }
        }
        // 移除已经执行完成的通知
