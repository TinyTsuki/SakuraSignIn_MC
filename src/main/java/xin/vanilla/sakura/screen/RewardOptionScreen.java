...
public class RewardOptionScreen extends Screen {
    ...
    private void renderRewardList() {
        ...
        for (String key : REWARD_BUTTONS.keySet()) {
            ...
            // 渲染物品图标
            operationButton.setBaseY(yOffset).render(keyManager);
        }
        // 渲染Tips
        for (String key : REWARD_BUTTONS.keySet()) {
            OperationButton operationButton = REWARD_BUTTONS.get(key);
            // 渲染物品图标
            operationButton.setBaseY(yOffset).renderPopup(keyManager);
        }
    }
