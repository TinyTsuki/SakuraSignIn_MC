...
public class SignInScreen extends Screen {
     ...
     * @param flag          是否处理过事件
     */
    private void handleOperation(OperationButton value, AtomicBoolean updateLayout, AtomicBoolean updateTexture, AtomicBoolean flag) {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        for (SignInCell cell : signInCells) {
            cell.render(super.font, keyManager);
        }
                ...
                for (SignInCell cell : signInCells) {
                    if (cell.isShowHover() && cell.isMouseOver(keyManager)) {
                        if (keyManager.onlyShiftPressed()) {
                            ...
                            cell.renderTooltip(super.font, this.itemRenderer, keyManager);
                        }
                    }
                }
            }
        ...
        // 显示开屏提示
            ...
            AbstractGuiUtils.fill(4, 4, super.width - 8, super.height - 8, 0xDD000000, 15);
            float x, y;
            ...
            super.buttons.stream().filter(button -> button instanceof Button
                    && (button.getMessage().equalsIgnoreCase(Text.translatable(EI18nType.OPTION, "confirm").getContent()))
                    || (button.getMessage().equalsIgnoreCase(Text.translatable(EI18nType.OPTION, "no_remind").getContent()))).forEach(button -> {
                if (button.getMessage().equalsIgnoreCase(Text.translatable(EI18nType.OPTION, "confirm").getContent())) {
                    button.x = (int) x;
                } else {
        ...
        if (popupOption.isHovered()) {
            ...
            BUTTONS.forEach((key, value) -> {
                if (value.isHovered() && value.isPressed()) {
                    this.handleOperation(value, updateLayout, updateTextureAndCoordinate, flag);
                }
