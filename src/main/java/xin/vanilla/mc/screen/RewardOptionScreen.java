...
public class RewardOptionScreen extends Screen {
                    ...
                    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                        ...
                        // 消息
                            ...
                            Minecraft.getInstance().setScreen(new StringInputScreen(this
                                    ...
                                    , new TextList(Text.i18n("请输入"))
                                    , new StringList("", "(0?1(\\.0{0,5})?|0(\\.\\d{0,5})?)?")
                                    , new StringList(((MutableComponent) RewardManager.deserializeReward(reward)).getString(), StringUtils.toFixedEx(reward.getProbability(), 5))
                                    , input -> {
                                if (CollectionUtils.isNotNullOrEmpty(input)) {
                                    MutableComponent textToComponent = AbstractGuiUtils.textToComponent(Text.literal(input.get(0)));
                                    BigDecimal p = StringUtils.toBigDecimal(input.get(1), BigDecimal.ONE);
    ...
    private Consumer<OperationButton.RenderContext> generateCustomRenderFunction(String content) {
        return context -> {
            ...
            AbstractGuiUtils.drawLimitedText(context.graphics(), super.font, getByZh(content), realX + 4, (int) (realY + (realHeight - super.font.lineHeight) / 2), (int) (realWidth - 22), 0xFFEBD4B1);
        };
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (SakuraSignIn.isRewardOptionBarOpened()) {
            graphics.drawString(super.font, getByZh("奖励规则类型"), 4, 5, 0xFFACACAC);
