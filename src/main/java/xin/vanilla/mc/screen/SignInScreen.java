...
public class SignInScreen extends Screen {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        for (SignInCell cell : signInCells) {
            cell.render(super.font, mouseX, mouseY);
        }
            ...
            for (SignInCell cell : signInCells) {
                if (cell.isShowHover() && cell.isMouseOver(mouseX, mouseY)) {
                    if ((this.keyCode == GLFW.GLFW_KEY_LEFT_SHIFT || this.keyCode == GLFW.GLFW_KEY_RIGHT_SHIFT) && this.modifiers == GLFW.GLFW_MOD_SHIFT) {
                        AbstractGuiUtils.drawPopupMessage(Text.i18n("鼠标左键签到\n右键补签/领取奖励").setFont(this.font).setAlign(Text.Align.CENTER), mouseX, mouseY, super.width, super.height);
                    } else {
        ...
        // 显示开屏提示
            ...
            super.buttons.stream().filter(button -> button instanceof Button
                    && (button.getMessage().equalsIgnoreCase(Text.i18n("确认").getContent()))
                    || (button.getMessage().equalsIgnoreCase(Text.i18n("不再提醒").getContent()))).forEach(button -> {
                if (button.getMessage().equalsIgnoreCase(Text.i18n("确认").getContent())) {
                    button.x = (int) x;
                } else {
