    ...
    public static void signIn(ServerPlayer player, SignInPacket packet) {
        ...
        if (ESignInType.REWARD.equals(packet.getSignInType())) {
            if (isRewarded(signInData, clientCompensateDate, false)) {
                player.sendSystemMessage(Component.translatable(getI18nKey("%s的奖励已经领取过啦"), DateUtils.toString(clientCompensateDate)));
                return;
            } else if (!isSignedIn(signInData, clientCompensateDate, false)) {
                player.sendSystemMessage(Component.translatable(getI18nKey("没有查询到[%s]的签到记录哦，鉴定为阁下没有签到！"), DateUtils.toString(clientCompensateDate)));
                return;
            } else {
        ...
        // 签到/补签
            ...
            player.sendSystemMessage(Component.translatable(getI18nKey("%s 签到成功, %s/%s"), DateUtils.toString(signInRecord.getCompensateTime()), signInData.getContinuousSignInDays(), getTotalSignInDays(signInData)));
        }
