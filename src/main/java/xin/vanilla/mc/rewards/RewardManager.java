    ...
    public static void signIn(ServerPlayer player, SignInPacket packet) {
        ...
        if (ESignInType.REWARD.equals(packet.getSignInType())) {
            if (isRewarded(signInData, clientDate, false)) {
                player.sendMessage(new TranslatableComponent(getI18nKey("%s的奖励已经领取过啦"), DateUtils.toString(clientDate)), player.getUUID());
                return;
            } else if (!isSignedIn(signInData, clientDate, false)) {
                player.sendMessage(new TranslatableComponent(getI18nKey("没有查询到[%s]的签到记录哦，鉴定为阁下没有签到！"), DateUtils.toString(clientDate)), player.getUUID());
                return;
            } else {
