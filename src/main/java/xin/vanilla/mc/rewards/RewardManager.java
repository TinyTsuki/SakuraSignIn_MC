...
public class RewardManager {
    ...
    public static void signIn(ServerPlayerEntity player, SignInPacket packet) {
        ...
        if (serverCompensateDateInt < signInDateInt && !DateUtils.equals(serverDate, clientDate, DateUtils.DateUnit.MINUTE)) {
            ...
            player.sendMessage(new TranslationTextComponent(getI18nKey("补签日期不早于服务器当前日期，补签失败")));
            return;
        ...
        if (ESignInType.RE_SIGN_IN.equals(packet.getSignInType()) && !ServerConfig.SIGN_IN_CARD.get()) {
            player.sendMessage(new TranslationTextComponent(getI18nKey("服务器未开启补签功能，补签失败")));
            return;
        ...
        if (ESignInType.REWARD.equals(packet.getSignInType())) {
            if (isRewarded(signInData, clientDate, false)) {
                player.sendMessage(new TranslationTextComponent(getI18nKey("%s的奖励已经领取过啦"), DateUtils.toString(clientDate)));
                return;
            } else if (!isSignedIn(signInData, clientDate, false)) {
                player.sendMessage(new TranslationTextComponent(getI18nKey("没有查询到[%s]的签到记录哦，鉴定为阁下没有签到！"), DateUtils.toString(clientDate)));
                return;
            } else {
                ...
                player.sendMessage(new TranslationTextComponent(getI18nKey("奖励领取成功")));
            }
        }
        // 签到/补签
        else {
