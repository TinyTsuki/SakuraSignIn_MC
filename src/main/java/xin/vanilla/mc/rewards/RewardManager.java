...
public class RewardManager {
    ...
    public static void signIn(ServerPlayer player, SignInPacket packet) {
        ...
        if (ESignInType.SIGN_IN.equals(packet.getSignInType()) && serverCompensateDateInt < clientCompensateDateInt) {
            ...
            player.sendMessage(new TranslatableComponent(getI18nKey("补签日期需早于服务器当前日期，补签失败")), player.getUUID());
            return;
        }
        // 判断签到CD
        ...
        if (ESignInType.REWARD.equals(packet.getSignInType())) {
            if (isRewarded(signInData, clientCompensateDate, false)) {
                player.sendMessage(new TranslatableComponent(getI18nKey("%s的奖励已经领取过啦"), DateUtils.toString(clientCompensateDate)), player.getUUID());
                return;
            } else if (!isSignedIn(signInData, clientCompensateDate, false)) {
                player.sendMessage(new TranslatableComponent(getI18nKey("没有查询到[%s]的签到记录哦，鉴定为阁下没有签到！"), DateUtils.toString(clientCompensateDate)), player.getUUID());
                return;
            } else {
        ...
        // 签到/补签
            ...
            signInData.setLastSignInTime(clientDate);
            ...
            player.sendMessage(new TranslatableComponent(getI18nKey("%s 签到成功, %s/%s"), DateUtils.toString(signInRecord.getCompensateTime()), signInData.getContinuousSignInDays(), getTotalSignInDays(signInData)), player.getUUID());
        }
