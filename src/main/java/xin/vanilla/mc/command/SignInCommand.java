...
public class SignInCommand {
    ...
    */
                    ...
                    if (indexToRemove < 0) {
                        ...
                        if (format.before(DateUtils.getServerDate())) {
                            ...
                            rewardKeyValue.getValue().getValue().forEach(reward -> {
                                IFormattableTextComponent detail = new StringTextComponent(reward.getName(true));
                                if (RewardManager.giveRewardToPlayer(player, signInData, reward)) {
                                    detail.withStyle(style -> style.withColor(net.minecraft.util.text.Color.fromRgb(java.awt.Color.GREEN.getRGB())));
                                } else {
                        ...
                        .then(Commands.literal("get")
                                ...
                                .then(Commands.literal("timeCoolingMethod")
                                        .executes(context -> {
                                            ...
                                            player.sendMessage(new TranslationTextComponent(getI18nKey("服务器签到时间冷却方式为: %s"), coolingMethod.name()), player.getUUID());
                                            return 1;
                                        })
                                )
                                ...
                                .then(Commands.literal("rewardAffectedByLuck")
                                        .executes(context -> {
                                            ServerPlayerEntity player = context.getSource().getPlayerOrException();
                                            player.sendMessage(new TranslationTextComponent(getI18nKey(String.format("服务器已%s奖励领取受幸运影响", ServerConfig.REWARD_AFFECTED_BY_LUCK.get() ? "启用" : "禁用"))), player.getUUID());
                                            return 1;
                                        })
                                )
                                .then(Commands.literal("continuousRewardsRepeatable")
                                        .executes(context -> {
                                            ServerPlayerEntity player = context.getSource().getPlayerOrException();
                                            player.sendMessage(new TranslationTextComponent(getI18nKey(String.format("服务器已%s连续签到奖励持续领取", ServerConfig.CONTINUOUS_REWARDS_REPEATABLE.get() ? "启用" : "禁用"))), player.getUUID());
                                            return 1;
                                        })
                                )
                                .then(Commands.literal("cycleRewardsRepeatable")
                                        .executes(context -> {
                                            ServerPlayerEntity player = context.getSource().getPlayerOrException();
                                            player.sendMessage(new TranslationTextComponent(getI18nKey(String.format("服务器已%s循环签到奖励持续领取", ServerConfig.CYCLE_REWARDS_REPEATABLE.get() ? "启用" : "禁用"))), player.getUUID());
                                            return 1;
                                        })
                                )
                        )
    ...
    private static void broadcastMessage(ServerPlayerEntity player, ITextComponent message) {
        player.server.getPlayerList().broadcastMessage(new TranslationTextComponent("chat.type.announcement", player.getDisplayName(), message), ChatType.SYSTEM, Util.NIL_UUID);
    }
